...
const skipFiles = /(?:package-lock|pnpm-lock|yarn\.lock|bun\.lockb|design-audit\.mjs|a11y-static-check\.mjs|token-audit\.mjs)$/;
...
for (const filePath of walk(root)) {
  ...
  if (vagueCtaCount >= 2) {
    warn('vague-cta-copy', file, 0, `${rel}: Several vague CTAs (${vagueCtaCount}). Prefer action copy tied to the product task.`);
  }

  const rounded2xlCount = countMatches(text, /\brounded-2xl\b/g);
  if (rounded2xlCount >= 6) {
    warn('repeated-large-radius', file, 0, `${rel}: Many rounded-2xl utilities (${rounded2xlCount}). Check for generic card-heavy styling.`);
  }

  const shadowXlCount = countMatches(text, /\bshadow-xl\b/g);
  if (shadowXlCount >= 3) {
    warn('heavy-shadow-overuse', file, 0, `${rel}: Multiple shadow-xl utilities (${shadowXlCount}). Use elevation deliberately and consistently.`);
  }

  const gradientCount = countMatches(text, /\bbg-gradient(?:-\w+)?\b/g);
  if (gradientCount >= 3) {
    warn('gradient-overuse', file, 0, `${rel}: Multiple background gradients (${gradientCount}). Check for generic AI-style decoration.`);
  }

  const cardComponentCount = countMatches(text, /<Card\b/g);
  if (cardComponentCount >= 8) {
    warn('card-heavy-screen', file, 0, `${rel}: Many Card components (${cardComponentCount}). Verify cards are not replacing real information architecture.`);
  }
  ...
  if (/(<button\b|<a\b|<input\b|<select\b|<textarea\b|role=["'](?:button|link|menuitem|tab)["'])/i.test(text) && !/\bfocus-visible\b|\bfocus:/i.test(text)) {
    warn('missing-focus-styling-signal', file, 0, `${rel}: Interactive UI detected without an obvious focus-visible/focus style signal.`);
  }
  ...
  if (/(animate-|transition-|duration-|framer-motion|motion\.|@keyframes)/i.test(text) && !/(prefers-reduced-motion|motion-reduce|useReducedMotion|reducedMotion)/i.test(text)) {
    warn('missing-reduced-motion-signal', file, 0, `${rel}: Motion or transitions detected without an obvious reduced-motion accommodation.`);
  }

  if (/\b(?:h-screen|min-h-screen)\b|height\s*:\s*100vh/i.test(text) && text.length > 8000) {
    warn('viewport-height-content-risk', file, 0, `${rel}: Fixed viewport-height utility appears in a content-heavy file. Check mobile overflow and covered content.`);
  }

  const mutedTextCount = countMatches(text, /\btext-(?:gray|slate|zinc|neutral)-[45]00\b/g);
  if (mutedTextCount >= 10) {
    warn('muted-text-overuse', file, 0, `${rel}: Many muted gray text utilities (${mutedTextCount}). Check contrast and hierarchy.`);
  }
  ...
  if (defaultContainerCount >= 3) {
    warn('repeated-default-container', file, 0, `${rel}: Repeated max-w-7xl mx-auto containers (${defaultContainerCount}). Check for generic template structure.`);
  }
}
